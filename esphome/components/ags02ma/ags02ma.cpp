...
void AGS02MAComponent::setup() {
  ESP_LOGCONFIG(TAG, "Setting up ags02ma...");

  auto version = this->read_version_();
  if (version) {
    ESP_LOGD(TAG, "AGS02MA Sensor Version: 0x%02X", *version);
    if (this->version_ != nullptr) {
      this->version_->publish_state(*version);
    }
  } else {
    ESP_LOGE(TAG, "AGS02MA Sensor Version: unknown");
  }

  auto resistance = this->read_resistance_();
  if (resistance) {
    ESP_LOGD(TAG, "AGS02MA Sensor Resistance: 0x%08" PRIX32, *resistance);
    if (this->resistance_ != nullptr) {
      this->resistance_->publish_state(*resistance);
    }
  } else {
    ESP_LOGE(TAG, "AGS02MA Sensor Resistance: unknown");
  }
...
void AGS02MAComponent::dump_config() {
  ...
  switch (this->error_code_) {
    ...
    case COMMUNICATION_FAILED:
      ESP_LOGE(TAG, "Communication with AGS02MA failed!");
      break;
    case CRC_CHECK_FAILED:
      ESP_LOGE(TAG, "The crc check failed");
      break;
    case ILLEGAL_STATUS:
      ESP_LOGE(TAG, "AGS02MA is not ready to return TVOC data or sensor in pre-heat stage.");
      break;
    case UNSUPPORTED_UNITS:
      ESP_LOGE(TAG, "AGS02MA returns TVOC data in unsupported units.");
      break;
    default:
...
/**
 * Sets new I2C address of AGS02MA.
 */
bool AGS02MAComponent::new_i2c_address(uint8_t newaddress) {
...
bool AGS02MAComponent::set_zero_point_with_factory_defaults() { return this->set_zero_point_with(ZP_DEFAULT); }

bool AGS02MAComponent::set_zero_point_with_current_resistance() { return this->set_zero_point_with(ZP_CURRENT); }

bool AGS02MAComponent::set_zero_point_with(uint16_t value) {
...
optional<uint32_t> AGS02MAComponent::read_tvoc_() {
  ...
  if (status_bit != 0) {
    ...
    ESP_LOGW(TAG, "Reading AGS02MA data failed: illegal status (not ready or sensor in pre-heat stage)!");
    return nullopt;
  }

  if (units != 0) {
    this->error_code_ = UNSUPPORTED_UNITS;
    ESP_LOGE(TAG, "Reading AGS02MA data failed: unsupported units (%d)!", units);
    return nullopt;
  }
...
optional<uint8_t> AGS02MAComponent::read_version_() {
...
optional<uint32_t> AGS02MAComponent::read_resistance_() {
...
template<size_t N> optional<std::array<uint8_t, N>> AGS02MAComponent::read_and_check_(uint8_t a_register) {
  auto data = this->read_bytes<N>(a_register);
  if (!data.has_value()) {
    this->error_code_ = COMMUNICATION_FAILED;
    ESP_LOGE(TAG, "Reading AGS02MA version failed!");
  ...
  if (crc_byte != calc_crc8_(res, len)) {
    ...
    ESP_LOGE(TAG, "Reading AGS02MA version failed: crc error!");
    return optional<std::array<uint8_t, N>>();
  }
...
template<size_t N> uint8_t AGS02MAComponent::calc_crc8_(std::array<uint8_t, N> dat, uint8_t num) {
